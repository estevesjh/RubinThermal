...
| `scripts/m2_tau_analysis.py` | Estimate M2 thermal time constant (tau) | `m2_tau_analysis.png`, `m2_average_equilibration.png` |
| `scripts/m2_tau_night_fraction.py` | M2 tau using normalized night fraction | `m2_equilibration_*.png` |
| `scripts/temp_correlation.py` | Outside vs M2 temperature correlation | `temp_correlation_*.png` |
| `scripts/temp_correlation_v2.py` | Correlation vs night fraction | `correlation_vs_night_fraction.png` |
...
Observing night CSVs are generated by `queryTemps/find_observing_nights.py`:
`/sdf/data/rubin/user/esteves/thermal_analysis/observing_nights_2025.csv`

### Dome open definition

The dome is considered **open** when both aperture shutters are in position [90, 110]:
- `positionActual0` in [90, 110] **AND**
- `positionActual1` in [90, 110]

Queried from `lsst.sal.MTDome.apertureShutter`. Sun altitude (`lsst.sal.MTPtg.skyEnvironment.sunAltitude`) is also queried alongside dome status.
