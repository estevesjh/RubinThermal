...
\caption{Tracking performance (top row) and gradient penalties (bottom row) for three operational scenarios: Conservative (honeycomb-safe), Moderate (balanced), and Aggressive (performance-focused). Red stars indicate optimal rate for each scenario. The shaded regions represent the interquartile range across 392 test nights.}
...
\begin{table}[H]
\centering
\caption{Tracking error compliance: percentage of time below temperature thresholds}
\label{tab:tracking_thresholds}
\begin{tabular}{@{}S[table-format=1.2]S[table-format=1.2]S[table-format=2.1]S[table-format=2.1]S[table-format=2.1]S[table-format=1.2]@{}}
\toprule
{Max Rate} & {RMS} & \multicolumn{3}{c}{Tracking Error Below Threshold} & {Gradient} \\
\cmidrule(lr){3-5}
{(\si{\celsius/h})} & {(\si{\celsius})} & {\% $<$\SI{0.3}{\celsius}} & {\% $<$\SI{0.5}{\celsius}} & {\% $<$\SI{1.0}{\celsius}} & {95th \%ile (\si{\celsius})} \\
\midrule
0.30 & 0.93 & 38.2 & 53.1 & 77.7 & 0.67 \\
0.50 & 0.62 & 55.1 & 71.6 & 90.7 & 0.90 \\
\rowcolor{yellow!20} 0.70 & 0.49 & 63.7 & 81.4 & 95.2 & 0.95 \\
1.00 & 0.40 & 68.7 & 87.3 & 97.4 & 0.97 \\
1.25 & 0.36 & 70.2 & 89.2 & 98.1 & 0.97 \\
1.50 & 0.34 & 70.7 & 90.1 & 98.5 & 0.98 \\
2.00 & 0.32 & 71.1 & 90.8 & 98.9 & 0.98 \\
\bottomrule
\end{tabular}
\vspace{0.5em}

\footnotesize\textit{Note: Highlighted row indicates current operating point (\SI{0.7}{\celsius/h}). Gradient 95th percentile shown for $\tau_{\text{gradient}} = \SI{3.0}{h}$.}
\end{table}

\textbf{Key findings from tracking threshold analysis:}
\begin{itemize}
    \item At \SI{0.7}{\celsius/h}: Tracking error stays below \SI{0.5}{\celsius} for \textbf{81.4\%} of the time, and below \SI{1.0}{\celsius} for \textbf{95.2\%} of the time
    \item Increasing to \SI{1.0}{\celsius/h} improves $<$\SI{0.5}{\celsius} compliance from 81\% to 87\%, with 95th percentile gradient increasing only slightly from 0.95 to \SI{0.97}{\celsius}
    \item At \SI{0.3}{\celsius/h}: Only 53\% of time below \SI{0.5}{\celsius} and 78\% below \SI{1.0}{\celsius} --- significant tracking degradation
    \item \textcolor{safegreen}{The 95th percentile gradient stays below \SI{1.0}{\celsius} for all rate configurations, indicating typical operation is within safety limits}
\end{itemize}
...
\begin{tabular}{@{}llccc@{}}
\toprule
Scenario & Description & $\tau_{\text{grad}}$ & Penalty Weight & Optimal Rate \\
\midrule
Conservative & Honeycomb-safe, worst-case & \SI{3.0}{h} & 2.0 & \SI{0.50}{\celsius/h} \\
Moderate & Balanced tradeoff & \SI{2.5}{h} & 1.0 & \SI{0.75}{\celsius/h} \\
Aggressive & Performance-focused & \SI{2.0}{h} & 0.5 & \SI{0.75}{\celsius/h}--\SI{1.0}{\celsius/h} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding}: The current operating point of \SI{0.7}{\celsius/h} is validated as near-optimal for the moderate scenario, confirming it represents a reasonable engineering tradeoff.
