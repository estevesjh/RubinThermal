#!/usr/bin/env python3
"""
Accurate sun position calculations for Cerro Pachón using astropy.

This module provides correct UTC-based sunset/sunrise times for the
Rubin Observatory location. All times are in UTC.
"""
...
def find_sun_event(date_str, event='sunset', target_alt=0.0):
    """
...
def get_sun_times_for_date(date, data_start_time):
    """
    ...
    tuple (sunset_hours, sunrise_next_hours)
        Both in hours from data_start_time
    """
...
def build_sun_events(dates, data_start_time):
    """
...
if __name__ == '__main__':
    # Test the module
    print("Testing sun_utils.py")
    print("="*60)

    test_dates = ['2024-01-15', '2024-03-11', '2024-06-15', '2024-09-15', '2024-11-22']

    print(f"\nSun times for Cerro Pachón ({LAT}°, {LON}°):")
    print("-"*60)

    for date_str in test_dates:
        sunset = get_sunset_utc(date_str)
        sunrise = get_sunrise_utc(date_str)
        print(f"{date_str}: Sunrise {sunrise.strftime('%H:%M')} UTC, Sunset {sunset.strftime('%H:%M')} UTC")
