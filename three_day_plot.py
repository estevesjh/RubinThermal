...
from sun_utils import get_sunset_utc, get_sunrise_utc, build_sun_events, LAT, LON
...
# Sun utilities imported from sun_utils module

def compute_rate(t_arr, T_arr, t, window=1.0):
    t_before = max(t - window/2, t_arr[0])
    t_after = min(t + window/2, t_arr[-1])
    if t_after <= t_before:
        return 0.0
    T_before = np.interp(t_before, t_arr, T_arr)
    T_after = np.interp(t_after, t_arr, T_arr)
...
T1 = -3  # Start of phase 2 (hours before sunset)
T2 = 0   # Start of phase 3 (at sunset)
...
# Get sun times for each day using sun_utils module
sun_events = []
print("\nComputing sun times:")
for day_offset in range(5):
    ...
    if sunrise_utc and sunset_utc:
        # Convert to hours from data start
...
# Simulate
t_sim = np.arange(sim_start_hours, sim_end_hours, DT)
n = len(t_sim)
...
for i in range(n):
    T_ambient[i] = get_temperature(t_sim[i])

# Initialize mirror at ambient - cold bias
T_mirror[0] = T_ambient[0] - COLD_BIAS
...
for i in range(n):
    ...
    if relevant_sunset is None:
        ...
        relevant_sunrise = sun_events[-1]['sunrise']
        next_sunrise = relevant_sunrise + 24
    ...
    if is_nighttime:
        ...
        weighted_temp = sum(w * get_temperature(t + dt) for w, dt in zip(weights, times))
        ...
        T_sunset_expected = get_temperature(relevant_sunset)
        fixed = T_sunset_expected - COLD_BIAS
        ...
        T_sunset_expected = get_temperature(relevant_sunset)
        T_setpoint[i] = T_sunset_expected - COLD_BIAS
        phase[i] = 1
...
ax.set_title('Three-Phase Thermal Control: 3-Day Simulation\n'
...
plt.tight_layout()
plt.savefig('three_phase_3day.png', dpi=150, bbox_inches='tight')
